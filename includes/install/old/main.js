...
function callbackIcebergModal(save) {
    var callback = _modal_options.callback.close;
    if (save === true) {
        callback = _modal_options.callback.save;
    }
...
function addDB() {
    ...
    if (dbhost!='' && dbport!='' && dbname!='' && dbuser!='') {
        ...
        $('#dbhost, #dbname, #dbuser, #dbpass').val('');
        $('#dbport').val('3306');
...
function nextStep3() {
    ...
    var $domains = $("#domains option");
    var $dbs = $("#dbs option");
    if ($email.val() == '') {$email.addClass('error');error=true;}
    else {$email.removeClass('error');}
    if ($username.val() == '') {$username.addClass('error');error=true;}
    else {$username.removeClass('error');}
    if ($password.val() == '') {$password.addClass('error');error=true;}
    else {$password.removeClass('error');}
    if ($dbs.length == 0) {$("#dbs").addClass('error');error=true;}
    else {$("#dbs").removeClass('error');}
    if (!error) {
        var list
        list = new Array;
        $domains.each(function(){list.push( $(this).val() );});
        list = JSON.stringify(list, null, 2);
        $('#domainslist').val(list);

        list = new Array;
        $dbs.each(function(){list.push( $(this).val() );});
        list = JSON.stringify(list, null, 2);
        $('#dbslist').val(list);
        ...
        openIcebergModal({
            title: step3_error.title,
            content: step3_error.content
        });
    }
}
